<template>
  <!-- 说明: 参数 uploadFileList 为数组的文件上传  const uploadFileList = ref([])
    demo1:
    ...
    demo2: 大小样式控制
    <div class="fei-pic-url  引用组件的地方使用class">
      <FeiUpload v-model="uploadFileList_fei01" :limit="2" :fileSize="1" />
    </div>
      ...
      /** 通过外部属性修改图片大小样式 */
      :deep(.fei-pic-url .el-upload-list--picture-card) {
        color: red;
        width: 70px;
        height: 70px;
      }
      :deep(.fei-pic-url .el-upload-list--picture-card .el-upload--picture-card) {
        color: greenyellow;
        width: 70px;
        height: 70px;
      }
      :deep(.fei-pic-url .el-upload-list--picture-card .el-upload-list__item) {
        color: black;
        width: 70px;
        height: 70px;
      }
